...
function CategoryOne({ wrapperClass, styleClass, buttonStatus }) {
  return (
    <>
      <div className={`row ${wrapperClass || 'g-5 mt--25'}`}>
        {items.map((data, i) => (
          <ScrollAnimation
            ...
            className={`col-lg-3 col-md-6 col-sm-6 col-12 ${styleClass || ''}`}
            animateOnce
            key={i}
          >
            <div className="service-card service-card-1 radius-small">
              <div className="inner">
                <div className="thumbnail">
                  <a href={data.link}>
                    <img
                      src={`${process.env.PUBLIC_URL}./images/category/category-01/${data.image}`}
                      alt="Category Thumb"
                    />
                  </a>
                </div>
